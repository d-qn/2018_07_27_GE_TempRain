...
plotSingleYear <- function(syear) {
  ...
  theme(
    plot.margin = margin(20, 3, 0, -2, "pt"), 
    plot.title = element_text(
      size = fontSize / 1.1, colour = "#242d75"),
    plot.subtitle = element_text(size = fontSize / 1.2)
  ) +
  scale_colour_identity() +
  labs(
    title = "154 années de température à Genève, 1864 - 2018",
    subtitle = "La courbe courbe pointillée correspond à la moyenne mensuelle de 1981 à 2010.\nL'animation ralentit pour les 10 années les plus chaudes"
  )
}
...
p <- bg_chart (base_size = fontSize) + 
  ...
  geom_text(
    data = shiftLabel, hjust = 0.5, vjust = 0, nudge_y = 0.15,
    aes(x = Month, y = y1, label = label), 
    size = fontSize / 3, colour = "#660000", family = font_tw_light
  ) +
  geom_text(
    data = timePLabel, 
...
ap <- p + 
  labs(title = "", subtitle = "") + transition_states(timeP, transition_length = 300, state_length = 400, wrap = T) +
  enter_fade() +
  exit_fade()

# apa <- animate(ap, width = 620 * 1.1, height = 700 * 1.1)
# anim_save("GE_temperatureAverageShift.gif")
...
vid %>% save_animation(file = "test6.mp4")
